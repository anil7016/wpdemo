...
class Icon_Box extends BDevs_El_Widget {
    ...
    protected function register_content_controls() {
        ...
        $this->add_control(
            'service_count',
            [
                'label' => __( 'Service Count', 'bdevselement' ),
                'type' => Controls_Manager::TEXT,
                'label_block' => true,
                'default' => __( '02', 'bdevselement' ),
                'placeholder' => __( 'Type Service Count', 'bdevselement' ),
                'dynamic' => [
                    'active' => true,
                ],
                'condition' => [
        ...
        $this->add_control(
            'service_label',
            [
                'label' => __( 'Service Label', 'bdevselement' ),
                'type' => Controls_Manager::TEXT,
                'label_block' => true,
                'default' => __( ' Happy Clients & Students ', 'bdevselement' ),
                'placeholder' => __( 'Type Service Count', 'bdevselement' ),
                'dynamic' => [
                    'active' => true,
                ],
    ...
    protected function register_style_controls() {
        ...
        $this->add_control(
            'desc_color',
            [
                'label' => __( 'Text Colors', 'bdevselement' ),
                'type' => Controls_Manager::COLOR,
                'selectors' => [
    ...
    protected function render() {
        ...
        <div class="fact v-fact-border text-center">
            <?php if(!empty($settings['service_count'])) : ?>
            <h1 class="counter-count"><span class="counter"><?php echo esc_html($settings['service_count']); ?></span>k+</h1>
            <?php endif; ?>
            <?php if(!empty($settings['service_label'])) : ?>
            <span><?php echo esc_html($settings['service_label']); ?></span>
            <?php endif; ?>
        </div>
        ...
        <?php else:
            $this->add_inline_editing_attributes( 'title', 'basic' );
            $this->add_render_attribute( 'title', 'class', 'browse__wrapper__title mb-25 bdevs-el-title' );
            if ( !empty($settings['button_link']) ) {
                $this->add_link_attributes( 'button', $settings['button_link'] );
            }
        ...
        <div class="browse-box bdevs-el-content">
            <div class="browse__wrapper">
                <?php if ( $settings['title' ] ) :
                    printf( '<%1$s %2$s><a href="'.$settings['title_link']['url'].'">%3$s</a></%1$s>',
                        tag_escape( $settings['title_tag'] ),
                        $this->get_render_attribute_string( 'title' ),
                        bdevs_element_kses_basic( $settings['title' ] )
                        );
                ...
                <?php if ( $settings['description'] ) : ?>
                <p class="browse__wrapper__subtitle mb-25" <?php $this->print_render_attribute_string( 'description' ); ?>><?php echo bdevs_element_kses_intermediate( $settings['description'] ); ?></p>
                <?php endif; ?>
